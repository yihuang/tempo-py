...
def write_docker_run_script(val_dir: Path, node_args: list[str]) -> Path:
    ...
    binary = _sh_quote(node_args[0])
    arg_lines = " \\\n".join("  " + _sh_quote(a) for a in node_args[1:])
    # Stream stdout+stderr to node.log in the (bind-mounted) node dir, so logs
    # persist and can be tailed live — mirroring supervisord's stdout_logfile.
    # Appended (>>) so they survive a container restart.
    script = f"""\
#!/bin/sh
