...
- **Native (supervisord)** — each validator runs as an OS process
- **Docker Compose** — each validator runs in an isolated container
  (``--gen-compose-file`` flag)

See [docs/devnet.md](docs/devnet.md) for full documentation: config format,
port schemes, network topology, CLI reference, config patches, Nix shell,
and all deployment options.
