...
NVNM Chain L1 internal testnet on GKE `mantra-chain-sandbox-asia-east2-std`.

---

## 1. Node roles

`[EMPIRICAL]` Tempo is a **single binary, single process** — execution (reth) and
...
|---|---|---|---|---|
| **validator** | — (runs consensus) | ed25519 + BLS share + enode | Yes — proposes & notarizes | ClusterIP only |
...
`[EMPIRICAL]` A follower "is a **full node**" — it verifies consensus
...
`[EMPIRICAL]` `crates/consensus/src/follow/mod.rs`: a node "syncs from an
upstream node (**validator or another follower**)". Chaining is first-class,
which is what makes the public tier possible.

> **Certification is on by default — do not disable it.** `[EMPIRICAL]`
> `bin/tempo/src/cli.rs` exposes `--follow.nocertify` as the opt-*out*
...
`[INFERRED]` The public tier is the internet-facing component and therefore the
...
`[EMPIRICAL]` `bin/tempo/src/p2p_proxy.rs` builds with `build_with_noop_provider`
...
`[EMPIRICAL]` Simplex "tolerates f Byzantine faults of 3f+1 validators (i.e. < 1/3)",
quorum `2f+1`. With `N=4`: `f=1`, quorum `3`. One validator may be down or faulty
without halting. `[EMPIRICAL]` SPIKE-0001 E1 sets `≥4` as the devnet minimum.

`[INFERRED]` Because validator identities cannot be replicated (GAP-2), `N` is the
...
graph TB
    ...
    subgraph gcp["GCP · mantra-chain-sandbox · asia-east2"]
        ...
        subgraph ns["namespace: nvnm-tempo-devnet-1"]
            ...
            subgraph valtier["validator — private, no ingress"]
                V0["validator-0<br/>:8000 consensus"]
                V1["validator-1"]
                V2["validator-2"]
                V3["validator-3"]
            end
    ...
    SM --> ESO
    ESO --> V0
    KMS -.->|"initContainer<br/>gcloud kms decrypt"| V0
...
|---|---|---|---|
| validator | validator | 8000/tcp | Consensus P2P, Simplex full mesh (TLS-auth) |
| validator | validator | 30303/tcp | Execution devp2p — tx gossip |
| rpc/internal | validator | 8546/tcp | **Blocks down** — WS `--follow` + finality certs |
| rpc/internal | validator | 30303/tcp | **Txs up** — devp2p gossip to proposers |
...
| all pods (initContainer) | `169.254.169.254` | 80/tcp | GKE metadata — Workload Identity token |
| all pods (initContainer) | Google APIs | 443/tcp | `gcloud kms decrypt` at pod start |
...
`[EMPIRICAL]` Port assignments from NVNMChain `docs/architecture/deploy.md`.
...
`[INFERRED]` Wiring only the block path yields a chain where `eth_blockNumber`
...
`[EMPIRICAL]` Tempo node-security guidance: "avoid running other internet-facing
services on the same host as your validator." `[INFERRED]` Putting `--http` on a
...
`[EMPIRICAL]` Single-region same-zone-region RTT in GCP `asia-east2` is ≤5 ms
...
`[EMPIRICAL]` "`synchrony-bound` must be ≥ `target-block-time` and at least `2s` to
...
> specifies `50ms`. `[INFERRED]` We keep `50ms` as deliberate headroom for GKE
...
> **NTP is a hard dependency.** `[EMPIRICAL]` Tempo requires `chrony` or `ntpd`,
> explicitly **not** `systemd-timesyncd`. On GKE COS nodes, time sync is handled by
...
`[EMPIRICAL]` Two secrets per validator, both "equivalent to a validator private key":
...
**Security properties `[INFERRED]` from this design:**

- Plaintext key material exists only in tmpfs inside the pod's own network/PID
  namespace, and only for the pod's lifetime.
...
**Residual risk `[EMPIRICAL]`:** Tempo has no HSM/KMS *signing* path — the node
...
> `[EMPIRICAL]` The Wiz runtime sensor is currently **archived/disabled** in the
...
graph LR
    ...
    subgraph std["asia-east2-std cluster"]
        NS["ns: nvnm-tempo-devnet-1"]
        V["StatefulSet ×4<br/>…-validator-0..3<br/>replicas: 1 each"]
        F["StatefulSet ×2<br/>…-rpc-internal (2)<br/>…-rpc-public (2)"]
        PM["PodMonitoring ×2<br/>consensus + execution"]
        HM["Emissary Host + Mapping"]
        CC["ComputeClass<br/>nvnm-validator-class<br/>on-demand only"]
    end
...
### Why one StatefulSet per validator (not one with `replicas: 4`)

`[INFERRED]` Each validator has a **distinct** `signing.share` and a distinct
on-chain identity. A single StatefulSet with 4 replicas would need per-ordinal
secret injection (the CometBFT pattern used by `cosmos-operator` via
`/etc/podinfo/ordinal`). That is possible, but:

| Approach | Pros | Cons |
|---|---|---|
| 1 STS × 4 replicas, ordinal-indexed secrets | Fewer objects; matches cosmos-operator habit | Rolling update touches all 4 → violates GAP-8 lockstep; one PDB for all; harder to pin a single validator to a node |
| **4 STS × 1 replica (chosen)** | Independent lifecycle per validator identity; per-validator PDB, node pinning, and upgrade gating; clearer blast radius | More YAML (mitigated by Helm `range`) |

The lockstep-upgrade constraint (GAP-8) is decisive: we need to stop, verify, and
start validators one at a time under operator control, which `OnDelete` on separate
StatefulSets makes explicit.

---

## 6. Observability

`[EMPIRICAL]` Two metrics endpoints per node, both scraped by GMP `PodMonitoring`:

| Port | Source | Key series |
|---|---|---|
| 8001 | Commonware consensus | `tempo_consensus_view`, `tempo_consensus_epoch`, `tempo_consensus_proposals_total`, `tempo_consensus_notarizations_total`, `tempo_consensus_finalizations_total`, `tempo_dkg_manager_ceremony_players`, `tempo_dkg_manager_ceremony_dealers`, `tempo_epoch_manager_how_often_signer_total` |
| 9001 | reth execution | reth standard set (block processing, txpool, peers, sync, db) |

Logs: `[EMPIRICAL]` structured JSON via `--log.file.directory`, level via `RUST_LOG`.
...
|---|---|---|---|
| Block production stalled | `rate(tempo_consensus_finalizations_total[5m]) == 0` | P1 | Chain halt |
| View churn | `deriv(tempo_consensus_view[5m])` ≫ expected | P2 | `[EMPIRICAL]` "frequent view increments → increase `wait-for-proposal` by 2× P95 RTT". `tempo_consensus_view` is a **gauge**, so use `deriv()`/`delta()`, not `rate()` |
| Validator not signing | `tempo_epoch_manager_how_often_signer_total` flat for 1 epoch | P2 | Validator dropped from active set |
| DKG ceremony short | `tempo_dkg_manager_ceremony_players < N` at epoch boundary | P1 | GAP-7 — reshare may fail |
| Quorum at risk | ready validators `<= 2f+1` (i.e. ≤3 of 4) | P1 | At exactly quorum, one more loss halts the chain — alert **before** the halt, not after |
| Clock skew | node NTP offset > 500 ms | P2 | `synchrony-bound` violation risk |
| PVC utilisation | `> 80%` | P3 | Growth ~22 GiB/day at full block rate |

`[INFERRED]` Route P1 to the existing OpsGenie channel
...
> Expressions above are written as plain PromQL for readability. Under Google
> Managed Prometheus the metric names are namespaced —
> `prometheus.googleapis.com/tempo_consensus_finalizations_total/counter` — so
> each must be translated when the alert policies are actually authored in
> Terraform (`_custom/terraform-gcp-alerts`).
...
`[EMPIRICAL]` Finality is deterministic and single-shot — no reorgs, no confirmation
depth. `[EMPIRICAL]` Target sub-second p50 (SPIKE-0001 E4 gate); this must be
...
| Area | Assessment |
|---|---|
| **Security / InfraSec** | New attack surface: tcp:8000 consensus P2P (mitigated — TLS-authenticated, on-chain identity allowlist, `bypass-ip-check false`, ClusterIP only); tcp:30303 devp2p (mitigated — NetworkPolicy restricts validator ingress to validators + internal tier); EVM RPC (mitigated — public tier only, Cloudflare-restricted, `debug`/`trace` enabled on internal but **not** public). Three tiers mean a compromised internet-facing node reaches only the internal tier, enforced by NetworkPolicy egress rather than config alone. Least privilege via per-node KSA → KMS binding. Residual: no HSM signing path; plaintext key in pod memory. |
| **Performance** | 500 ms blocks need `e2-highmem-8` minimum (8 vCPU meets the "8+ cores" floor but is *below* the "16+ recommended for validators"). `[INFERRED]` Acceptable for a zero-load devnet; revisit to `n2-standard-16`/`c3` for anything load-tested. pd-ssd IOPS scale with size — 200 Gi gives ~6k IOPS, adequate at devnet volume. |
...
`[EMPIRICAL]` Tempo floor: 8+ cores, 32 GiB, NVMe 1 TiB+.
`[INFERRED]` The limits above fit `e2-highmem-8` (8 vCPU / 64 GiB) with one
